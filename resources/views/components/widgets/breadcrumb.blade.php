...
if (!isset($currentLevel[$segment])) break;
...
<ol class="ms-3 flex items-center whitespace-nowrap mb-4">
    @foreach ($breadcrumbs as $crumb)
        <li class="flex items-center text-sm text-gray-800 dark:text-gray-50">
                ...
                <div x-data="{ open: false }" class="relative" @mouseover="open = true" @mouseleave="open = false">
                    ...
                    <div x-show="open" x-cloak x-transition
                        ...
                        @foreach($crumb['children'] as $child)
                            <div class="relative" x-data="{ subOpen: false }" @mouseover="subOpen = true" @mouseleave="subOpen = false">
            ...
            @elseif($crumb['url'])
                {{-- Regular Link --}}
                <a href="{{ $crumb['url'] }}" class="hover:text-gray-900 dark:hover:text-gray-300 transition-colors">
                    {{ trans($child['name']) }}
                </a>
            @else
                {{-- Current Page --}}
            ...
            @if(!$loop->last)
                @if(trans('string.lang direction') == 'rtl')
                    <x-svg.arrow-right class="shrink-0 mx-3 overflow-visible size-2.5 text-slate-400 dark:text-slate-500" />
                @else
