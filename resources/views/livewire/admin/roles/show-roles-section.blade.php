...
<section class="bg-white container mx-auto md:w-5/5 lg:w-4/5   mt-8 rounded-lg  dark:bg-gray-900">
    <div class="w-full px-4 py-8 mx-auto ">
        <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">{{__("string.Roles")}} : </h2>
        @if ($status !== [])
                    ...
                    @foreach ($roles as $role)
                        <tr class="border-b dark:border-gray-700" wire:key="{{ $role->id }}">
                            ...
                            <td class="px-4 py-3 flex flex-row md:justify-center gap-2">
                                <x-button.primary wire:click="index({{$role->id}})" type="button" class="w-min">
                                    <x-svg.arrow-up />
                                </x-button.primary>
                                <x-button.danger type="button" wire:click="delete({{ $role->id }})"
                                    wire:confirm="{{__('message.confirm delete role')}}" class="w-min">
                                    <x-svg.trash />
